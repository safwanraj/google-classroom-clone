// from ownwer side assignment creation and updation
...
router.put("/return", async (req, res, next) => {
    ...
    await Task.findByIdAndUpdate(taskId, { $set: { marksScored: mark } })
...
router.get("/:classId/:assignmentId/returned", async (req, res, next) => {
    ...
    await Task.find({ assignmentId: assignmentId, classId: classId, marksScored: { $ne: 0 } })
    .populate("userId")
...
router.get("/:classId/:assignmentId/submissions", async (req, res, next) => {
    ...
    await Task.find({ assignmentId: assignmentId, classId: classId, marksScored: { $eq: 0 } })
    .populate("userId")
    .then(tasks => res.status(200).send(tasks))
    .catch(err => res.status(400).send(err))
})
...
router.get("/:classId/:assignmentId/all", async (req, res, next) => {
    ...
    await Task.find({ assignmentId: assignmentId, classId: classId })
    .populate("userId")
    .then(tasks => res.status(200).send(tasks))
    .catch(err => res.status(400).send(err))
})
